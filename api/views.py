...
class PasswordResetAPI(generics.CreateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        def send(user, email_to, token):
            ...
            email["To"] = email_to
            email["Subject"] = "Restablecer contraseña ALKNOS"

            uid_bytes = str(user.id).encode('ascii')
            uidb64_bytes = base64.b64encode(uid_bytes)
            uidb64 = uidb64_bytes.decode('ascii')

            content = "http://localhost:3000/reset?token=" + uidb64.replace("=","%3D") + "%2F" + token
        ...
        thread = threading.Thread(target=send, args=(user,email, pwd_reset_token))
        thread.start()
